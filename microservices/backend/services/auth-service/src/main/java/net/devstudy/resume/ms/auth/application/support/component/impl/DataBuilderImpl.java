package net.devstudy.resume.ms.auth.application.support.component.impl;
...
public class DataBuilderImpl implements DataBuilder {
    ...
    public String buildProfileUid(String firstName, String lastName) {
        String first = normalizeUidPart(firstName);
        String last = normalizeUidPart(lastName);
        String joined;
        if (first.isEmpty()) {
            joined = last;
    ...
    private String normalizeUidPart(String name) {
        if (name == null) {
            return "";
        }
